...
public:
  ...
  void build(size_t* opts = nullptr) {
    ...
    debugLog("StateMap after State_table init");
    size_t idx = 0;
    for (uint32_t i = 0;i < num_states;++i) {
      State& st = states[i];
      st.n[0] = st.n[1] = 0;
    ...
    debugLog("StateMap after first for loop");
    ...
    debugLog("StateMap after second for loop");
    optimizeStates();
    debugLog("StateMap after optimizeStates");
    debugLog("StateMap build end");
  }
};
